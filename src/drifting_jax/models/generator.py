...
from drifting_jax.distributed import enforce_ddp
from drifting_jax.runtime import ddp_rand_func
import math
from drifting_jax.env import HF_REPO_ID, HF_ROOT
...
class DitGen(nn.Module):
    ...
    def __call__(
        ...
        rng_x = rng_labels = None
        if noise is None or noise_labels is None:
            rng_x, rng_labels = random.split(self.make_rng('noise'))

        expected_noise_shape = (B, self.input_size, self.input_size, self.in_channels)
        if noise is None:
            if B % jax.device_count() != 0:
                x = random.normal(rng_x, expected_noise_shape)
            else:
                x = ddp_rand_func(shard="ddp", rand_type="normal")(rng_x, expected_noise_shape)
        else:
            x = enforce_ddp(jnp.asarray(noise))
            if x.shape != expected_noise_shape:
                raise ValueError(f"noise must have shape {expected_noise_shape}, got {x.shape}")
        x = x * temp + jnp.zeros_like(c, dtype=x.dtype)[:, None, None, None] # move sharding!

        if self.use_bf16:
            x = x.astype(jnp.bfloat16)

        expected_label_shape = (B, max(1, self.noise_coords))
        if noise_labels is None:
            noise_labels = random.randint(rng_labels, expected_label_shape, 0, max(1, self.noise_classes))
        else:
            noise_labels = enforce_ddp(jnp.asarray(noise_labels))
            if noise_labels.shape != expected_label_shape:
                raise ValueError(
                    f"noise_labels must have shape {expected_label_shape}, got {noise_labels.shape}"
                )
        noise_labels = noise_labels + jnp.zeros_like(c, dtype=noise_labels.dtype)[:, None] # move sharding!
...
def load_hf(
    ...
    *,
    ...
    """
    from drifting_jax.models.hf import load_generator_jax
